...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  drawClientBlock(page, font, [
    ...
    [invoice.customer?.billingZip, invoice.customer?.billingCity].filter(Boolean).join(" "),
  ...
  // Totaux
  yRef.y -= 10;
  const sub = toNum(invoice.subTotal);
  const tvat = toNum(invoice.taxTotal);
  const ttc = toNum(invoice.grandTotal);
